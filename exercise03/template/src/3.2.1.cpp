...
extern void globalMemCoalescedKernel_Wrapper(dim3 gridDim, dim3 blockDim, int *d_src, int *d_dst, int numElements);
extern void globalMemStrideKernel_Wrapper(dim3 gridDim, dim3 blockDim, int *d_src, int *d_dst, int numElements, int stride);
extern void globalMemOffsetKernel_Wrapper(dim3 gridDim, dim3 blockDim, int *d_src, int *d_dst, int numElements, int offset);
...
main ( int argc, char * argv[] )
{
    ...
    if (optMemorySize == 0) { 
        int numElements = optGridSize * optBlockSize + optOffset;
        optMemorySize = numElements * sizeof(int);
        std::cout << "*** Ignoring size parameter; using kernel launch parameters and offset, size=" << optMemorySize << std::endl; 
    } else {
        std::cout << "*** Using specified size parameter for offset test, size=" << optMemorySize << std::endl;
    }
	} else {
	...
	if ( chCommandLineGetBool ( "global-coalesced", argc, argv ) ) {
        grid_dim.x = 1;
        std::cout << "*** Note: For 'global-coalesced' test, grid_dim is forced to 1." << std::endl;
    }

    int numElements = optMemorySize / sizeof(int);
    ...
    if ( !optUsePinnedMemory ) { // Pageable
		...
		cudaHostAlloc((void**)&h_memoryA, static_cast<size_t>(optMemorySize), cudaHostAllocPortable);
    	cudaHostAlloc((void**)&h_memoryB, static_cast<size_t>(optMemorySize), cudaHostAllocPortable);
    }
    ...
    for ( int i = 0; i < optMemCpyIterations; i ++ ) {
        // D2H copy
		cudaMemcpy(h_memoryB, d_memoryA, optMemorySize, cudaMemcpyDeviceToHost);
    ...
    for ( int i = 0; i < optNumIterations; i++ ) {
        ...
        //
        if ( chCommandLineGetBool ( "global-coalesced", argc, argv ) ) {
			globalMemCoalescedKernel_Wrapper(grid_dim, block_dim, d_memoryA, d_memoryB, numElements);
        } else if ( chCommandLineGetBool ( "global-stride", argc, argv ) ) {
            globalMemStrideKernel_Wrapper(grid_dim, block_dim, d_memoryA, d_memoryB, numElements, optStride);
        } else if ( chCommandLineGetBool ( "global-offset", argc, argv ) ) {
            globalMemOffsetKernel_Wrapper(grid_dim, block_dim, d_memoryA, d_memoryB, numElements, optOffset);
        } else {
            break;
        }
	...
	double totalDataBytes = static_cast<double>(optMemorySize) * 2.0;
    ...
    if ( chCommandLineGetBool ( "global-coalesced", argc, argv ) ) {
        ...
        std::cout.precision ( 2 );
        std::cout << ", bw=" << std::fixed << std::setw(6) << (totalDataBytes / kernelTimer.getTime(optNumIterations)) / (1E09) << " GB/s" << std::endl;
    }
    ...
    if ( chCommandLineGetBool ( "global-offset", argc, argv ) ) {
        ...
        //std::cout << ", time=" << kernelTimer.getTime(optNumIterations) << 
        std::cout.precision ( 2 );
         double actualDataBytes = static_cast<double>(numElements - optOffset) * sizeof(int) * 2.0;
    std::cout << ", bw=" << std::fixed << std::setw(6) 
              << actualDataBytes / kernelTimer.getTime(optNumIterations) / (1E09) 
              << "GB/s" << std::endl;
    }
